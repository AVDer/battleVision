...
struct UnitGeneralInfo {
  ...
  UnitGeneralInfo(uint32_t u_id, uint32_t o_id, uint32_t a, unit_type_t u_t, std::string&& n)
      : unit_id(u_id), opponent_id(o_id), amount(a), unit_type(u_t), name(n) {}
...
struct UnitDrawInfo {
  ...
  UnitDrawInfo(point_t pos, color_t c, shape_t sh, rect_size_t s, angle_t a, unit_state_t us)
      : position(pos), color(c), shape(sh), size(s), angle(a), state(us) {}
...
class UnitInfo {
 public:
  ...
  void set_angle(angle_t angle) { unit_draw_info_.angle = angle; }
